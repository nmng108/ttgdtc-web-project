			...
			<div class="row">
				<div class="container table-responsive-lg" style="width: 75vw;">
							...
							for ($i = 0; $i < count($cart); $i++) { 
								...
								<tr class="<?=$class_value?>" id="row_item_<?=$item['itemCode']?>">
									...
									<td class="delete-button">
										<button style=" background-color:#F5DEB3; color: black ;"class="btn btn-danger" id="delete_button_<?=$item['itemCode']?>" onclick="delete_item(this)" hidden>Xóa</button>
									</td>
									
									<td class="edit-button">
										<button style=" background-color: #63B8FF; color: black;"class="btn btn-warning" id="edit_button_<?=$item['itemCode']?>" onclick="switch_edit_mode(this)">Sửa</button>
									</td>
								</tr>
								<?php
							}
					...
					<a href="./checkout?cat=<?=SPORT_EQUIPMENT?>" class="checkout-button">
						<button style="background-color: #F5DEB3; color: black" class="btn btn-success" id="checkout_button_<?=SPORT_EQUIPMENT?>" style="align-content: right; color: goldenrod" onclick="return process_cart(this, <?=$_SESSION[USERID]?>)">Tiếp tục</button>
					</a>
				</div>
			</div>
