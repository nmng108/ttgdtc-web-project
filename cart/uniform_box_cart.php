			...
			<div class="row">
				<div class="container  table-responsive-lg" style="width: 75vw;">
							...
							for ($i = 0; $i < count($cart); $i++) { 
								...
								<tr class="<?=$class_value?>" id="item_<?=$item['itemCode']?>" style="text-align:center;">
									...
									<td class="delete-button">
										<button style="background-color: #F5DEB3;" class="btn btn-danger" id="delete_button_<?=$item['itemCode']?>" onclick="delete_item(this)" hidden>Xóa</button>
									</td>
									
									<td class="edit-button">
										<button style="background-color: #63B8FF;"class="btn btn-warning" id="edit_button_<?=$item['itemCode']?>" onclick="switch_edit_mode(this)">Sửa</button>
									</td>
								</tr>
								<?php
							}
					...
					<a href="./checkout?cat=<?=UNIFORM?>" class="checkout-button">
						<button class="btn checkout-button" id="checkout_button_<?=UNIFORM?>" style="align-content: right; background-color: #F5DEB3; color: black;" onclick="return process_cart(this)">Tiếp tục</button>
					</a>
				</div>
			</div>
